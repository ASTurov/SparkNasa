...
spark-submit --class SparkStart --master yarn --deploy-mode client --executor-memory 1G --num-executors 3 app.jar hadoop-master /nasa/Aug

start-master.sh && start-slaves.sh
spark-submit --class SparkStart --master spark://hadoop-master:7077 --total-executor-cores 8 app.jar hadoop-master /nasa/Aug
